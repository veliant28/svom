...
# Core tables for product search/linking (brand + article + cross + category links + fitment).
TABLES=(
  articles
  article_numbers
  article_prd
...
STATE_FILE=".autodb_clone_sync.linking_core.state"
WAIT_FOR_AUTODB=120
BATCH_SIZE=0
LIMIT_ROWS=0
PROGRESS_EVERY=20
...
usage() {
  cat <<'EOF'
Usage:
  ./run_autodb_clone_sync_linking_core.sh [options]

Clone only core Auto_DB_Pro tables required for product search/linking.
Resume support:
  1) Between tables via local state file (.autodb_clone_sync.linking_core.state)
...
log "Linking-core clone run started. state_file=$STATE_FILE start_index=$START_INDEX"
log "Tables: ${TABLES[*]}"
...
for ((i=START_INDEX; i<TABLE_COUNT; i++)); do
  ...
  if "${cmd[@]}" 2>&1 | while IFS= read -r line; do log "[$table] $line"; done; then
    ...
    log "State kept at index=$i. Resume later with: ./run_autodb_clone_sync_linking_core.sh"
    exit 1
  fi
done

total_elapsed="$(( $(date +%s) - RUN_STARTED_AT ))"
log "Linking-core clone run finished successfully. total_elapsed=${total_elapsed}s"

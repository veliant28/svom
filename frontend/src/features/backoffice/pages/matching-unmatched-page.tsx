...
import {
  bulkAutoMatchBackoffice,
  bulkIgnoreBackoffice,
  getBackofficeUnmatchedOffers,
...
export function MatchingUnmatchedPage() {
  const t = useTranslations("backoffice.matching.unmatched");
  ...
  const queryFn = useCallback(
    (token: string) =>
      getBackofficeUnmatchedOffers(token, {
        q,
        supplier,
        brand,
      }),
  ...
  async function bulkAutoMatch() {
    if (!token || selectedIds.length === 0) return;
    try {
      await bulkAutoMatchBackoffice(token, { raw_offer_ids: selectedIds });
      showSuccess(t("messages.bulkAutoMatched", { count: selectedIds.length }));
      setSelectedIds([]);
      await refetch();
    } catch (error: unknown) {
      showApiError(error, t("messages.actionFailed"));

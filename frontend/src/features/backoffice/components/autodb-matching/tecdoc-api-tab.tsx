...
function resolveQuotaMetrics(quota: AutoDbRemoteQuota | null): { used: number; limit: number; remaining: number } {
  const used = Math.max(toCount(quota?.estimated_queries_used), 0);
  const limitFromField = Math.max(toCount(quota?.estimated_limit_per_hour), 0);
  const limitFromRemaining = used + Math.max(toCount(quota?.estimated_queries_remaining), 0);
  const usagePercent = Math.max(0, Math.min(toCount(quota?.usage_percent), 100));
  const limitFromPercent = usagePercent > 0 ? Math.round((used * 100) / usagePercent) : 0;
  const limit = Math.max(limitFromField, limitFromRemaining, limitFromPercent, 1);
              ...
              <div className="mt-2 grid grid-cols-2 gap-2 text-xs" style={{ color: "var(--muted)" }}>
                ...
                <div>
                  ...
                  <p className="font-medium" style={{ color: "var(--text)" }}>
                    {quotaPaused ? formatCountdown(toCount(monitor.remoteQuota?.seconds_until_reset)) : t("tecdocApi.badges.quotaLive")}
                  </p>
                </div>
              </div>
            </div>

...
from apps.autodb.services.matching import AutoDbSafeLinkPlanner


class Command(MatchingCommandMixin, BaseCommand):
    help = "Guarded safe link apply command skeleton. Requires --apply and only reads safe_link_candidate jobs."
    command_name = "autodb_matching_apply_safe_links"
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            planner = AutoDbSafeLinkPlanner()
            rows = [planner.apply_job(job, apply=True) for job in jobs]
            csv_path, md_path, rows_count = self.export(
                run=run,
                rows=rows,
                title="Auto_DB Matching Safe Link Apply Gate",
                export_prefix=str(options.get("export_prefix") or ""),
                product_writes=False,
                foundation_note="foundation command does not mutate Product link fields",
            )

...
class Command(MatchingCommandMixin, BaseCommand):
    help = "Plan safe product link candidates with overwrite protections."
    command_name = "autodb_matching_plan_safe_links"
    ...
    def handle(self, *args, **options):
        ...
        try:
            rows = AutoDbMatchingPipelineService().plan_safe_links(run=run, limit=int(options["limit"]), dry_run=bool(options["dry_run"]))
            csv_path, md_path, rows_count = self.export(
                run=run,
                rows=rows,
                title="Auto_DB Matching Safe Link Plan",
                export_prefix=str(options.get("export_prefix") or ""),
                product_writes=False,
            )
            self.finish_run(run, rows_count=rows_count, product_writes=False)

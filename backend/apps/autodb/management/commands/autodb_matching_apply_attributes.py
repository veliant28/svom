...
class Command(MatchingCommandMixin, BaseCommand):
    help = "Guarded attributes apply command skeleton. Requires --apply; images disabled."
    command_name = "autodb_matching_apply_attributes"
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            rows = AutoDbEnrichmentPlanner().apply_attributes(jobs, apply=True)
            csv_path, md_path, rows_count = self.export(
                run=run,
                rows=rows,
                title="Auto_DB Matching Attributes Apply Gate",

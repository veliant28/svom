...
from apps.catalog.services.manual_remaining_categories import (
    REMAINING_MANUAL_CATEGORY_SPECS,
    STATUS_NEEDS_REVIEW,
    STATUS_SAFE,
...
class Command(BaseCommand):
    help = "Assign controlled whitelist manual categories for remaining uncategorized products."
    ...
    def handle(self, *args, **options):
        ...
        expected_targets = len(REMAINING_MANUAL_CATEGORY_SPECS)
        if len(target_categories) < expected_targets:
            raise CommandError("Remaining manual categories are missing. Run seed_remaining_manual_categories first.")

        products = self._load_products(supplier_code=supplier_code, limit=limit)
        ...
        self.stdout.write(
            "assign_remaining_manual_categories started "
        ...
        for product in products:
            ...
            if decision.status != STATUS_SAFE or decision.confidence < min_confidence:
                ...
                rows.append(self._row_for(product=product, offer=offer, status="skip_unclear", reason=decision.reason, proposed=decision.proposed_category, confidence=decision.confidence))
                continue
        ...
        self.stdout.write("assign_remaining_manual_categories summary:")
    ...
    def _load_target_categories(self) -> dict[str, Category]:
        ...
        return {
            item.slug: item
            for item in Category.objects.filter(slug__in=slugs, source=Category.SOURCE_MANUAL)
        }

    def _load_products(self, *, supplier_code: str, limit: int) -> list[Product]:
        target_slugs = [item.slug for item in REMAINING_MANUAL_CATEGORY_SPECS]
        qs = (
            ...
            .filter(Q(category__isnull=True) | Q(category__slug__in=target_slugs))
            .distinct()
            .order_by("id")
        )
        if limit > 0:

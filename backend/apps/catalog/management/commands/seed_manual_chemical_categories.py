...
class Command(BaseCommand):
    help = "Seed controlled manual child categories for car chemicals under curated root."
    ...
    def handle(self, *args, **options):
        ...
        if parent is None:
            raise CommandError(
                f"Parent category not found for manual chemicals: expected slug='{MANUAL_CHEMICAL_ROOT_SLUG}'"
            )
        ...
        self.stdout.write("seed_manual_chemical_categories summary:")
    ...
    def _resolve_parent(self) -> Category | None:
        parent = Category.objects.filter(slug=MANUAL_CHEMICAL_ROOT_SLUG, parent__isnull=True).first()
        if parent:
            return parent
        return Category.objects.filter(name="Автохимия и аксессуары", parent__isnull=True).first()

    def _simulate(self, *, parent: Category, summary: SeedSummary) -> None:
        for spec in MANUAL_CHEMICAL_CATEGORY_SPECS:
    ...
    def _count_duplicate_slugs(self) -> int:
        slugs = [item.slug for item in MANUAL_CHEMICAL_CATEGORY_SPECS]
        return max(Category.objects.filter(slug__in=slugs).count() - len(slugs), 0)

    def _count_duplicate_names(self, *, parent: Category) -> int:
        target_names = {" ".join(item.name.split()).casefold() for item in MANUAL_CHEMICAL_CATEGORY_SPECS}

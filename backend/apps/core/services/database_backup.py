...
class DatabaseBackupService:
    ...
    FILE_PREFIX = "svom-postgresql"

    def dispatch_due_backup(self, *, now: datetime | None = None) -> DatabaseBackupDispatchResult:
    ...
    def close_stale_running_backup(
        ...
        *,
        ...
        instance = backup_settings or get_database_backup_settings()
    ...
    def run_backup(self) -> DatabaseBackupResult:
        backup_settings = get_database_backup_settings()
        ...
        try:
            ...
            backup_path = backup_dir / self._build_backup_filename(moment=moment)

            self._run_pg_dump(backup_path=backup_path)

            if not backup_path.exists():
                raise RuntimeError("pg_dump finished without creating a backup file.")

            backup_size = backup_path.stat().st_size
            deleted_paths = self._apply_retention(backup_dir=backup_dir, retention_count=backup_settings.retention_count)
            finished_at = dj_timezone.now()
            message = f"PostgreSQL backup created: {backup_path.name}"
            ...
            self._clear_run_lock()

    def _run_pg_dump(self, *, backup_path: Path) -> None:
    ...
    def _apply_retention(self, *, backup_dir: Path, retention_count: int) -> list[str]:
        keep_count = max(int(retention_count or 0), 1)
        backups = sorted(
            backup_dir.glob(f"{self.FILE_PREFIX}-*.dump"),
    ...
    def _build_backup_filename(self, *, moment: datetime) -> str:
        database = settings.DATABASES["default"]
        ...
        return f"{self.FILE_PREFIX}-{db_name}-{timestamp}.dump"
    ...
    def _acquire_run_lock(self) -> bool:
        try:
            return bool(cache.add("core:database_backup:run", "1", timeout=self._timeout_seconds()))
        except Exception:
            return True

    def _clear_run_lock(self) -> None:
        try:
            cache.delete("core:database_backup:run")
        except Exception:
            pass
